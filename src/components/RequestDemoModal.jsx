...
const RequestDemoModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Map to existing demo_requests table structure
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 sm:px-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-[#0f1620] border border-[#d4b04c]/20 rounded-2xl shadow-2xl shadow-[#d4b04c]/10 overflow-hidden"
          >
            ...
            <div className="p-8">
              {status === 'success' ? (
                ...
                <>
                  <h2 className="text-2xl font-bold text-white mb-2 font-serif">Request a Demo</h2>
                  <p className="text-gray-400 mb-6 text-sm">
                    See how AIP can transform your infrastructure financing.
                  </p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5">Company / Organization</label>
                      <input type="text" name="companyName" value={formData.companyName} onChange={handleChange} className="w-full bg-[#151a21] border border-white/10 rounded-lg px-4 py-3 text-white focus:border-[#d4b04c] focus:outline-none transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5">Message (Optional)</label>
                      <textarea name="message" rows="3" value={formData.message} onChange={handleChange} className="w-full bg-[#151a21] border border-white/10 rounded-lg px-4 py-3 text-white focus:border-[#d4b04c] focus:outline-none transition-all resize-none" />
                    </div>
                    ...
                    <Button type="submit" disabled={loading} className="w-full bg-[#d4b04c] text-[#0b0f14] hover:bg-white hover:text-[#0b0f14] font-bold py-6 text-lg rounded-lg shadow-lg shadow-[#d4b04c]/10">
                      {loading ? <><Loader2 className="w-5 h-5 animate-spin mr-2" /> Processing...</> : "Request Demo"}
                    </Button>
                  </form>
                </>
              )}
