...
const ExplorePlatformAccessModal = () => {
  const { activeModal, closeModal } = useFormModal();
  const isOpen = activeModal === 'platformAccess';
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      setStatus('idle');
      setFormData({ fullName: '', email: '', organization: '', useCase: '' });
      setErrorMessage('');
    }, 300);
  };
              ...
              {status === 'success' ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400 mb-6">
                    Platform access details coming soon. We'll be in touch!
                  </p>
                ...
                <>
                  <h2 className="text-2xl font-bold text-white mb-2 font-serif">Explore Platform Access</h2>
                  <p className="text-gray-400 mb-6 text-sm">
                    Discover how AIP connects projects with capital.
                  </p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5">Primary Interest / Use Case</label>
                      <textarea name="useCase" rows="3" value={formData.useCase} onChange={handleChange} className="w-full bg-[#151a21] border border-white/10 rounded-lg px-4 py-3 text-white focus:border-[#d4b04c] focus:outline-none transition-all resize-none" placeholder="I am an investor looking for..." />
                    </div>
                    ...
                    <Button type="submit" disabled={loading} className="w-full bg-[#d4b04c] text-[#0b0f14] hover:bg-white hover:text-[#0b0f14] font-bold py-6 text-lg rounded-lg shadow-lg shadow-[#d4b04c]/10">
                      {loading ? <><Loader2 className="w-5 h-5 animate-spin mr-2" /> Processing...</> : "Request Access"}
                    </Button>
                  </form>
                </>
              )}
...
export default ExplorePlatformAccessModal;